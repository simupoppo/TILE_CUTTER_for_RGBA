...
class povray_template():
    ...
    def write_file(self,out):
        with open(out,mode="w") as f:
            ...
            f.write('object{merge{box{<0-0.1,paksize/8,0-0.1>,<paksize*int_z/2+0.1,paksize*int_y,paksize*int_x/2+0.1>}box{<0-0.11,0+paksize/128,paksize*int_x/4>,<paksize*int_z/2+0.11,paksize*int_y,paksize*int_x/2+0.11>}}}\n')
            f.write('#else\n')
            f.write('object{box{<-paksize,-paksize*int_y,-paksize>,<paksize*(max(int_x,int_z)+2)/2+0.1,paksize*int_y,paksize*(max(int_z,int_x)+2)/2>}}\n')
            f.write('#end\n')
